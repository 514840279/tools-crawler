      ...
      <div>
        <el-descriptions title="文件信息" direction="vertical" :column="7" :border="true">
          ...
          <el-descriptions-item label="跳过头行">
            <el-input-number v-model="fileInfo.skip" :min="0" />
          </el-descriptions-item>
          <el-descriptions-item label="分割符号" v-if="fileInfo.fileType == FileType.TXT || fileInfo.fileType == FileType.CSV">
            <el-input v-model="fileInfo.separator" placeholder="分割符号：通常以逗号','，'tab'分割字段" />
          </el-descriptions-item>
          <el-descriptions-item label="限定符号" v-if="fileInfo.fileType == FileType.TXT || fileInfo.fileType == FileType.CSV">
            <el-input v-model="fileInfo.enclosed" placeholder="限定符号:通常以单引号，双引号，反引号包围字段" />
          </el-descriptions-item>
          <el-descriptions-item label="首行标题">
            <el-switch v-model="fileInfo.hasHead" class="ml-2" inline-prompt style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949" active-text="Y" inactive-text="N" />
          </el-descriptions-item>
        </el-descriptions>
        <div style="text-align: center;margin-top:8px">
          <el-button type="primary">确认配置信息</el-button>
...
import { SysLoadFileInfo, FileType } from '../../../interface/LoadFile.js';
...
// let localFile = ref<SysLoadFileInfo>();
let datas = ref<Array<Map<string, string>>>();
