                        ...
                        <el-table-column v-for="(column, ind) in activeColumns" :key="ind" :prop="column.name" :label="column.title" :width="column.width == null ? '' : column.width" :align="column.align == null ? 'left' : column.align" :resizable="column.resizable == null ? false : column.resizable">
                            <template #default="scope">
                                <div v-if="column.type == SearchType.OPERATION">
                                    <el-button @click="onClickRow(scope.$index, scope.row, column.name)" size="small">{{ scope.row[column.name] }}</el-button>
                                </div>
                            </template>
                        </el-table-column>
...
// 初始化参数
function init(): void {
    localOptionBtn.value = parents.optionBtn;
    ...
    showColumns.value.forEach((item, index) => {
        ...
        if (item.search != null) {
            // 默认值设置
            let searchType = item.type == null ? SearchType.TEXT : item.type;
            // 添加可选条件
            searchColumns.push({
        ...
        http.post<any>(url.page, param).then((reponse) => {
            if (reponse.data != null && reponse.code == 200) {
                ...
                if (size > 0 && dataList.value.length == 0) {
                    param.pageNumber = (param.pageNumber - 1) > 1 ? param.pageNumber - 1 : 1;
                    initTable();
                }
